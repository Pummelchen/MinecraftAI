...
- `scripts/backup_releases_local.py` - mirrors release `server-files/` and
  ...
  `Backup/Client_26.1.2_2026-06-06_V1.zip`. Non-version release IDs are skipped so
...
- `Backup/Client_26.1.2_2026-06-06_V1.zip`
...
Live operator permissions stay at level 4 and `max-tick-time=-1` through
`server-config/server.properties.override` so `/locate structure` remains
available on the server even when a modded structure scan takes longer than the
vanilla watchdog limit. A long locate can still pause the server tick while it
searches; the policy is to preserve the command instead of watchdog-killing the
JVM.

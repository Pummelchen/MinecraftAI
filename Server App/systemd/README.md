...
- `caddy.service.d/pummelchen-hardening.conf` augments the official packaged Caddy service with restart policy, kernel/control-group protections, address-family restrictions, and read-only access to the public site tree. Keep the packaged Caddy unit as the service owner.
...
The Caddy drop-in requires the public tree at `/var/minecraftai/web/site/public` to exist before service start. Validate `/etc/caddy/Caddyfile`, run `systemd-analyze verify caddy.service`, and confirm the `caddy` account can traverse and read the site tree before a cutover.

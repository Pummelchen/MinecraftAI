...
`add-mod` is now a full pipeline command: when you call `MCPummelchenModServer add-mod` without `--dry-run true`, it resolves dependencies, applies compatibility checks, runs a server smoke test, builds a full release, runs the MC-version-scoped DMG build + 1-minute headless live soak, and publishes a DMG-backed release for download. Each generated macOS app is dedicated to one Minecraft server version and carries that version in the app name, such as `MCPummelchenModClient 26.2.app`.

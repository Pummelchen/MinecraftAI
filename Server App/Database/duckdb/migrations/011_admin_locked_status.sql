-- Treat admin-locked mods as accepted active mods in reporting and release accounting.
-- Admin Locked means the operator explicitly requires the mod to carry forward;
-- validation must work around it instead of silently pruning it.
...
SELECT
  ...
  SUM(
    CASE
      WHEN lower(COALESCE(m.active_status, '')) IN ('active', 'ok', 'priority mod', 'admin locked') THEN 1
      ELSE 0
    END
  ) AS active_mod_count,
